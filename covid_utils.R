...
plot_novos_por_dia <- function(dados, novos_metrica, media_metrica,
  ...
  if (log_y) {
    p <- p +
      scale_y_log10(breaks = 10^(0:5), limits = c(1, NA),
...
plot_novos_por_grupo <- function(dados, novos_metrica, media_metrica,
  ...
  if (log_y) {
    p <- p + 
      scale_y_log10(expand = c(0.05, 0.05), breaks = 10^(0:5),
                    limits = c(1, NA),
                    labels = trans_format("log10", math_format(10^.x)))
  } else {
    p <- p + 
...
plot_rt <- function(dados) {
  df <- dados %>%
    filter(type == "nowcast") 
  p <-  ggplot(df, aes(date, median)) +
    geom_ribbon(aes(ymin = lower_90, ymax = upper_90), alpha = 0.5,
                fill = "#f4cfce") +
    geom_line(alpha = 0.9, size = 0.8) +
    geom_hline(aes(yintercept = 1), lty = "dashed") +
    geom_text(aes(y = median, label = round(median, digits = 3)),
              nudge_x = 1, size = 4, hjust = "left", fontface = "bold",
              data = filter(df, date == max(date))) +
    scale_x_date("Dia", date_breaks = "2 weeks", date_labels = "%d %b",
                 expand = expansion(c(0, 0.08))) +
    scale_y_continuous("Taxa de reprodução R(t)", limits = c(0, 3),
                       expand = c(0, 0)) +
    theme(panel.grid.minor = element_blank(),
...
plot_rt_por_grupo <- function(dados) {
  df <- dados %>%
    filter(type == "nowcast") 
  p <-  ggplot(df, aes(date, median)) +
    geom_hline(aes(yintercept = 1), lty = "dashed", alpha = 0.6) +
    geom_ribbon(aes(ymin = lower_90, ymax = upper_90), alpha = 0.5,
                fill = "#f4cfce") +
    geom_line(alpha = 0.9, size = 0.7) +
    geom_text(aes(y = last(median), label = round(median, digits = 3)),
              nudge_x = 1, size = 3, hjust = "left", fontface = "bold",
              vjust = "bottom", data = filter(df, last_nowcast)) +
    scale_x_date("Dia", date_breaks = "month", date_labels = "%b",
                 expand = expansion(c(0, 0.16))) +
    scale_y_continuous("Taxa de reprodução R(t)", limits = c(0, NA),
                       expand = c(0, 0)) +
    #facet_wrap(~region_code, ncol = 4) +
